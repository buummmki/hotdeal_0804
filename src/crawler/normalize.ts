...
const KEYWORD_RULES: [RegExp, string][] = [
  ...
  [/쿠폰|기프티콘|기프트카드|상품권|바우처|이용권|구독권|할인권|캐시백|페이백|무료\s*배포|무료\s*나눔|네이버페이.*(?:적립|라방)|일일\s*적립/i, 'coupon'],

  // 게임·SW
  [/스팀|steam|플스|ps5|ps4|엑스박스|xbox|닌텐도|스위치\s?2|에픽게임|험블|dlc|게임\s*세일|게임패스|겜스톱|스팀덱/i, 'game'],

  // PC·부품
  [/ssd|nvme|hdd|ddr\d|램\s?\d|메모리카드|cpu|라이젠|인텔\s|그래픽카드|rtx|gtx|rx\s?\d{3}|메인보드|파워서플라이|(?:pc|컴퓨터)\s?케이스|수랭|공랭|쿨러|모니터|기계식\s*키보드|게이밍\s*마우스|노트북|그램\s?1|맥북|데스크탑|본체|x3d|\d{4}\s?ti\b/i, 'pc'],

  // 디지털·모바일
  [/에어팟|버즈|갤럭시\s?(?:s|z|워치|탭)|아이폰|아이패드|태블릿|스마트워치|이어폰|헤드폰|헤드셋|충전기|충전\s*케이블|보조배터리|샤오미|미밴드|블루투스|공유기|외장하드|외장\s*케이스|usb|c타입/i, 'digital'],
  ...
  [/삼겹살|오겹살|목살|소고기|돼지고기|한우|닭갈비|닭가슴살|족발|보쌈|곱창|전골|갈비|스테이크|햄|스팸|참치|장어|오징어|새우|연어|생선|김치|만두|라면|사발면|비빔면|짜장|짬뽕|볶음밥|밀키트|즉석|햇반|죽\b|국\b|탕\b|찌개|피자|치킨|떡볶이|버거|샌드위치|빵|파리바게뜨|뚜레쥬르|베이커리|케이크|과자|팝콘|젤리|초콜릿|아이스크림|빙수|사탕|견과|아몬드|호두|커피|원두|음료|주스|생수|삼다수|콜라|펩시|사이다|우유|요거트|치즈|계란|쌀\b|잡곡|과일|복숭아|멜론|수박|포도|사과|딸기|바나나|채소|양파|감자|고구마|김\b|고추장|된장|간장|식용유|올리브유|간식|반찬|식품|먹거리|유산균|락토핏|프로바이오틱|비타민|오메가|홍삼|프로틴|단백질|영양제|건강식품/i, 'food'],

  // 생활·패션·뷰티
  [/물티슈|화장지|휴지|기저귀|세제|섬유유연제|샴푸|린스|바디워시|치약|칫솔|면도날|로션|스킨|에센스|크림|선크림|마스크팩|화장품|향수|의자|책상|가구|침대|매트리스|이불|베개|수건|주방|밀폐용기|프라이팬|냄비|텀블러|보온병|우산|가방|지갑|캐리어|옷\b|티셔츠|셔츠|바지|자켓|재킷|패딩|원피스|양말|속옷|신발|운동화|런닝화|슬리퍼|샌들|나이키|아디다스|골프|등산|캠핑|장난감|완구|피규어|반려|사료|고양이|강아지|필터|건전지/i, 'life'],
];
